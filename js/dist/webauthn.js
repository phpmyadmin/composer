...
/* harmony import */ var _modules_ajax_message_js__WEBPACK_IMPORTED_MODULE_2__ = __webpack_require__(10);
...
const handleCreation = $input => {
  ...
  navigator.credentials.create({
    ...
    $form.trigger('submit');
  }).catch(error => (0,_modules_ajax_message_js__WEBPACK_IMPORTED_MODULE_2__.ajaxShowMessage)(error, false, 'error'));
};
...
_modules_ajax_js__WEBPACK_IMPORTED_MODULE_1__.AJAX.registerOnload('webauthn.js', function () {
  if (!navigator.credentials || !navigator.credentials.create || !navigator.credentials.get || !window.PublicKeyCredential) {
    (0,_modules_ajax_message_js__WEBPACK_IMPORTED_MODULE_2__.ajaxShowMessage)(window.Messages.webAuthnNotSupported, false, 'error');
    return;
  }
  const $creationInput = jquery__WEBPACK_IMPORTED_MODULE_0__('#webauthn_creation_response');
  if ($creationInput.length > 0) {
    handleCreation($creationInput);
  }
  const $requestInput = jquery__WEBPACK_IMPORTED_MODULE_0__('#webauthn_request_response');
...
/******/ __webpack_require__.O(0, [49], function() { return __webpack_exec__(103); });
