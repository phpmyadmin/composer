"use strict";
(self["webpackChunkphpmyadmin"] = self["webpackChunkphpmyadmin"] || []).push([["server/status/monitor"],{

/***/ "./resources/js/modules/functions/chartByteFormatter.ts":
...
/* harmony import */ var _modules_functions_createProfilingChart_ts__WEBPACK_IMPORTED_MODULE_5__ = __webpack_require__("./resources/js/modules/functions/createProfilingChart.ts");
/* harmony import */ var _modules_functions_escape_ts__WEBPACK_IMPORTED_MODULE_6__ = __webpack_require__("./resources/js/modules/functions/escape.ts");
...
function serverResponseError() {
  bootstrap__WEBPACK_IMPORTED_MODULE_1__.Modal.getOrCreateInstance('#serverResponseErrorModal').show();
}
...
_modules_ajax_ts__WEBPACK_IMPORTED_MODULE_2__.AJAX.registerOnload('server/status/monitor.js', function () {
  const $jsDataForm = jquery__WEBPACK_IMPORTED_MODULE_0___default()('#js_data');
 ...
 */
_modules_ajax_ts__WEBPACK_IMPORTED_MODULE_2__.AJAX.registerTeardown('server/status/monitor.js', function () {
...
_modules_ajax_ts__WEBPACK_IMPORTED_MODULE_2__.AJAX.registerOnload('server/status/monitor.js', function () {
  ...
  jquery__WEBPACK_IMPORTED_MODULE_0___default()('#monitorChartColumnsSelect').on('change', function () {
    ...
    let numColumns;
    let $tr = jquery__WEBPACK_IMPORTED_MODULE_0___default()('#chartGrid').find('tr').first();
  ...
  jquery__WEBPACK_IMPORTED_MODULE_0___default()('#monitorAddNewChartButton').on('click', function (event) {
    jquery__WEBPACK_IMPORTED_MODULE_0___default()('#addChartButton').on('click', function () {
      const type = jquery__WEBPACK_IMPORTED_MODULE_0___default()('input[name="chartType"]:checked').val();
      if (type === 'preset') {
    ...
    const $presetList = jquery__WEBPACK_IMPORTED_MODULE_0___default()('#addChartModal').find('select[name="presetCharts"]');
  ...
  function monitorImportConfigImportEventHandler() {
    const input = jquery__WEBPACK_IMPORTED_MODULE_0___default()('#monitorImportConfigModal').find('#import_file')[0];
    ...
    reader.onload = function (e) {
      ...
      try {
        ...
        alert(window.Messages.strFailedParsingConfig);
        bootstrap__WEBPACK_IMPORTED_MODULE_1__.Modal.getOrCreateInstance('#monitorImportConfigModal').hide();
        return;
      }
  ...
  function loadLogVars() {
    ...
    jquery__WEBPACK_IMPORTED_MODULE_0___default().post('index.php?route=/server/status/monitor/log-vars', vars, function (data) {
      ...
      if (typeof data !== 'undefined' && data.success === true) {
        ...
        bootstrap__WEBPACK_IMPORTED_MODULE_1__.Modal.getOrCreateInstance('#monitorInstructionsModal').hide();
        serverResponseError();
        return;
      }
      let icon = (0,_modules_functions_getImageTag_ts__WEBPACK_IMPORTED_MODULE_7__["default"])('s_success');
      let msg = '';
      let str = '';
      ...
      $dialog.find('a.set').on('click', function () {
        const nameValue = jquery__WEBPACK_IMPORTED_MODULE_0___default()(this).attr('href').split('-');
        loadLogVars({
  ...
  jquery__WEBPACK_IMPORTED_MODULE_0___default()('input[name="chartType"]').on('change', function () {
    ...
    const title = jquery__WEBPACK_IMPORTED_MODULE_0___default()('input[name="chartTitle"]').val();
  ...
  function initGrid() {
    ...
    if ((0,_modules_functions_isStorageSupported_ts__WEBPACK_IMPORTED_MODULE_8__["default"])('localStorage')) {
      ...
      if (runtime.charts !== null && typeof window.localStorage.monitorVersion !== 'undefined' && monitorProtocolVersion !== window.localStorage.monitorVersion) {
        bootstrap__WEBPACK_IMPORTED_MODULE_1__.Modal.getOrCreateInstance('#incompatibleMonitorConfigModal').show();
      }
    }
    ...
    const numCharts = jquery__WEBPACK_IMPORTED_MODULE_0___default()('#chartGrid').find('.monitorChart').length;
  ...
  function addChart(chartObj) {
    ...
    chartObj.chart = new window.Chart('gridChartCanvas' + runtime.chartAI, {
      ...
      options: {
        locale: _modules_common_ts__WEBPACK_IMPORTED_MODULE_4__.CommonParams.get('lang').replace('_', '-'),
  ...
  function getLogAnalyseDialog(min, max) {
    ...
    function logAnalyseModalSlowLogEventHandler() {
      bootstrap__WEBPACK_IMPORTED_MODULE_1__.Modal.getOrCreateInstance(logAnalyseModal).hide();
    ...
    logAnalyseModal.addEventListener('shown.bs.modal', function () {
      ...
      (0,_modules_functions_ts__WEBPACK_IMPORTED_MODULE_3__.addDatepicker)($dateStart, 'datetime', {
        showMillisec: false,
        showMicrosec: false,
        timeFormat: 'HH:mm:ss',
        firstDay: window.firstDayOfCalendar
      });
      (0,_modules_functions_ts__WEBPACK_IMPORTED_MODULE_3__.addDatepicker)($dateEnd, 'datetime', {
        showMillisec: false,
        showMicrosec: false,
        timeFormat: 'HH:mm:ss',
        firstDay: window.firstDayOfCalendar
    ...
    bootstrap__WEBPACK_IMPORTED_MODULE_1__.Modal.getOrCreateInstance(logAnalyseModal).show();
  }
  function loadLog(type, min, max) {
    const dateStart = Date.parse(jquery__WEBPACK_IMPORTED_MODULE_0___default()('#logAnalyseDialog').find('input[name="dateStart"]').datepicker('getDate').toString()) || min;
    const dateEnd = Date.parse(jquery__WEBPACK_IMPORTED_MODULE_0___default()('#logAnalyseDialog').find('input[name="dateEnd"]').datepicker('getDate').toString()) || max;
    loadLogStatistics({
      src: type,
      start: dateStart,
      end: dateEnd,
  ...
  function loadLogStatistics(opts) {
    ...
    logRequest = jquery__WEBPACK_IMPORTED_MODULE_0___default().post(url, {
      ...
      if (typeof data !== 'undefined' && data.success === true) {
        ...
        bootstrap__WEBPACK_IMPORTED_MODULE_1__.Modal.getOrCreateInstance(analysingLogsModal).hide();
        serverResponseError();
        return;
      }
      if (logData.rows.length === 0) {
        bootstrap__WEBPACK_IMPORTED_MODULE_1__.Modal.getOrCreateInstance(analysingLogsModal).hide();
        bootstrap__WEBPACK_IMPORTED_MODULE_1__.Modal.getOrCreateInstance('#analysingLogsNoDataFoundModal').show();
        return;
      }
      ...
      function analysingLogsLogDataLoadedModalJumpEventHandler() {
        bootstrap__WEBPACK_IMPORTED_MODULE_1__.Modal.getOrCreateInstance(analysingLogsLogDataLoadedModal).hide();
      ...
      bootstrap__WEBPACK_IMPORTED_MODULE_1__.Modal.getOrCreateInstance(analysingLogsModal).hide();
      bootstrap__WEBPACK_IMPORTED_MODULE_1__.Modal.getOrCreateInstance(analysingLogsLogDataLoadedModal).show();
    });
    /**
    ...
    function filterQueries(varFilterChange) {
      let textFilter;
      const val = jquery__WEBPACK_IMPORTED_MODULE_0___default()('#filterQueryText').val();
      if (val.length === 0) {
        textFilter = null;
      } else {
        try {
      ...
      let i = 0;
      let q;
      const noVars = jquery__WEBPACK_IMPORTED_MODULE_0___default()('#noWHEREData').prop('checked');
      ...
      if (varFilterChange) {
        if (noVars) {
          let numCol;
          let row;
          const $table = jquery__WEBPACK_IMPORTED_MODULE_0___default()('#logTable').find('table tbody');
  ...
  function buildLogTable(data, groupInserts) {
    ...
    const cols = [];
    const $table = jquery__WEBPACK_IMPORTED_MODULE_0___default()('<table class="table table-striped table-hover align-middle sortable"></table>');
    let $tBody;
    let $tRow;
    let $tCell;
    // @ts-ignore
    ...
    if (amountColumn.text().indexOf('#') > -1) {
      ...
      new bootstrap__WEBPACK_IMPORTED_MODULE_1__.Tooltip(amountColumn.get(0), {
  ...
  function openQueryAnalyzer() {
    ...
    bootstrap__WEBPACK_IMPORTED_MODULE_1__.Modal.getOrCreateInstance(queryAnalyzerModal).show();
  }
  ...
  function loadQueryAnalysis(rowData) {
    ...
    jquery__WEBPACK_IMPORTED_MODULE_0___default().post('index.php?route=/server/status/monitor/query', {
      ...
      let explain = '<b>' + window.Messages.strExplainOutput + '</b> ' + jquery__WEBPACK_IMPORTED_MODULE_0___default()('#explain_docu').html();
      ...
      const tempExplain = function (key, value) {
        let newValue = value === null ? 'null' : (0,_modules_functions_escape_ts__WEBPACK_IMPORTED_MODULE_6__.escapeHtml)(value);
      ...
      jquery__WEBPACK_IMPORTED_MODULE_0___default()('#queryAnalyzerDialog').find('div.placeHolder a[href*="#showExplain"]').on('click', function () {
        const id = jquery__WEBPACK_IMPORTED_MODULE_0___default()(this).attr('href').split('-')[1];
      ...
      if (data.profiling) {
        ...
        for (i = 0, l = data.profiling.length; i < l; i++) {
          ...
          totalTime += duration;
          numberTable += '<tr><td>' + data.profiling[i].state + ' </td><td> ' + (0,_modules_functions_ts__WEBPACK_IMPORTED_MODULE_3__.prettyProfilingNum)(duration, 2) + '</td></tr>';
        }
        // Only put those values in the pie which are > 2%
        for (i = 0, l = data.profiling.length; i < l; i++) {
          duration = parseFloat(data.profiling[i].duration);
          if (duration / totalTime > 0.02) {
            chartData.push([(0,_modules_functions_ts__WEBPACK_IMPORTED_MODULE_3__.prettyProfilingNum)(duration, 2) + ' ' + data.profiling[i].state, duration]);
          } else {
            otherTime += duration;
          }
        }
        if (otherTime > 0) {
          chartData.push([(0,_modules_functions_ts__WEBPACK_IMPORTED_MODULE_3__.prettyProfilingNum)(otherTime, 2) + ' ' + window.Messages.strOther, otherTime]);
        }
        numberTable += '<tr><td><b>' + window.Messages.strTotalTime + '</b></td><td>' + (0,_modules_functions_ts__WEBPACK_IMPORTED_MODULE_3__.prettyProfilingNum)(totalTime, 2) + '</td></tr>';
        ...
        profilingChart = (0,_modules_functions_createProfilingChart_ts__WEBPACK_IMPORTED_MODULE_5__["default"])('queryProfilingCanvas', chartData, 'right');
      }
    });
